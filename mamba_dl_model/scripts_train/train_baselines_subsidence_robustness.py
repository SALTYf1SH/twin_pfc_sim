# -*- coding: utf-8 -*-
"""
Robustness Check Script for Subsidence Baselines (Multi-Seed).
Based on: train_baselines_subsidence.py

Methods:
1. Adds --seed argument.
2. Changes OUTPUT_DIR to 'robustness_results_subsidence_baselines'.
...
print(f"\n>>> Robustness Training (Subsidence): Model={MODEL_TYPE}, Seed={args.seed} <<<\n")

# Paths
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATASET_DIR = os.path.join(BASE_DIR, "../../final_dataset")
# Robustness Output Dir
OUTPUT_DIR = os.path.join(BASE_DIR, "../robustness_results_subsidence_baselines")
...
def train_baseline(model, train_loader, val_loader):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if avg_val_ssim > best_ssim:
            ...
            path = os.path.join(OUTPUT_DIR, f"best_baseline_subsidence_{MODEL_TYPE}_seed{args.seed}.pth")
