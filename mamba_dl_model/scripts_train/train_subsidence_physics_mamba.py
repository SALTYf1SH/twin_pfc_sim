...
if args.ablation_name == "full":
    OUTPUT_DIR = os.path.join(BASE_DIR, "../trained_models_subsidence_physics_mamba")
else:
    # Append custom ablation name if provided (e.g. for sensitivity analysis)
    OUTPUT_DIR = os.path.join(BASE_DIR, f"../trained_models_subsidence_{args.ablation_name}")
...
class TheoryConsistentLoss(nn.Module):
    ...
    def forward(self, pred, target, mining_distances, phys_params):
        ...
        # [Reverted to Original: Foreground Weighting]
        # Active Zone: (1 + 5) = 6x Weight
        # Background: 1x Weight
        # This preserves the continuous tail of the subsidence field.
        diff = (pred - target) ** 2
...
class DualBranchMambaModel(nn.Module):
    def __init__(self, static_size, dynamic_size, output_size, branch_mode='dual',
        ...
        # Add handling for branch_mode
        self.branch_mode = branch_mode
        pass # Partial init update, logic in forward pass mostly
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if (val_loss/n_val) < best_val_loss:
            ...
            path = os.path.join(OUTPUT_DIR, f"best_subsidence_{args.ablation_name}_{args.branch_mode}.pth")
