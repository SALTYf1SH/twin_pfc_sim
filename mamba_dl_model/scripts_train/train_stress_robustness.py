# -*- coding: utf-8 -*-
"""
Robustness Check Script for Stress Inversion (Multi-Seed).
Based on: train_stress_physics_mamba.py

Changes:
1. Adds --seed argument.
2. Changes OUTPUT_DIR to separate 'robustness_results_stress'.
...
OUTPUT_DIR = os.path.join(BASE_DIR, "../robustness_results_stress")
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if (val_loss/n_val) < best_val_loss:
            ...
            path = os.path.join(OUTPUT_DIR, f"best_stress_{suffix}_seed{args.seed}.pth")
            torch.save(model.state_dict(), path)

# --- 5. Main ---

def main():
    print(f"=== Robustness Check: Stress Inversion (Seed {args.seed}) ===")
    ...
    # To save time in robustness check, we can re-calculate or load existing. 
    # For robustness, let's re-calculate to be safe, but it takes time. 
    # Optimization: Check if main stats exist, if so copy them? No, let's just calc (it's fast enough).
    ...
    stats_path = os.path.join(OUTPUT_DIR, f"stress_stats_seed{args.seed}.pt")
    torch.save({'mean': mean, 'std': std}, stats_path)
    print(f"Stats saved to: {stats_path}") 
    
    transform = NormalizeTransform(mean, std)
    
    np.random.shuffle(all_files) # Shuffle determines split, seeded by args.seed
