...
def evaluate_loop(exp_config, val_loader, device, feature_dims, min_gt_pcr):
    ...
    if ablation == 'full':
        # Proposed (Physics)
        model_name = "best_subsidence_full_dual.pth"
        model_path = os.path.join(BASE_DIR, "../trained_models_subsidence_physics_mamba", model_name)
    else:
        # Ablation
        model_name = f"best_subsidence_{ablation}_{branch}.pth"
        model_path = os.path.join(MODEL_DIR, model_name)
...
def main():
    ...
    for exp in EXPERIMENTS:
        r = evaluate_loop(exp, val_loader, DEVICE, feature_dims, args.min_gt_pcr)
        if r: results.append(r)
    
    print("\nSaving CSV...")
    with open(SUMMARY_CSV, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=["Model", "MSE", "MAE", "SSIM", "PCC", "Evo", "PCR"])
        writer.writeheader()
