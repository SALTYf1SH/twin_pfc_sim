...
EXPERIMENTS = [
    {"name": "Proposed (Mamba)", "ablation": "full",         "branch": "dual"},
    {"name": "No Physics",       "ablation": "no_physics",   "branch": "dual"},
    {"name": "Static Only",      "ablation": "static_only",  "branch": "static_only"},
    {"name": "Dynamic Only",     "ablation": "dynamic_only", "branch": "dynamic_only"},
]
...
def evaluate_single_experiment(exp_config, val_loader, device, feature_dims, min_gt_pcr):
    ...
    # Construct Filename based on actual naming convention
    if ablation == "full":
        # Proposed Model - Check Physics Dir
        # Proposed Model - Check Mamba Physics Dir
        model_name = "best_stress_full_dual.pth"
        model_path = os.path.join(BASE_DIR, "../trained_models_stress_physics_mamba", model_name)
    else:
        # Ablation Models
...
def main():
    ...
    for exp in EXPERIMENTS:
        res = evaluate_single_experiment(exp, val_loader, DEVICE, feature_dims, args.min_gt_pcr)
