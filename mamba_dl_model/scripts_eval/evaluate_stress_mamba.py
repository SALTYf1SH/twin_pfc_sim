# -*- coding: utf-8 -*-
"""
Physics-Informed STRESS Model Evaluation Script (FINAL FIXED V2)

Fixes applied:
1. [Bug Fix] Dataset now returns 'filename' (6 items), fixing the silent crash in Visualization Mode.
2. [Normalization] Loads 'stress_stats.pt' for correct scaling (prevents MSE explosion).
3. [Physics] Uses ODE-based Mask Generator & Correct Transpose.
"""
...
DEFAULT_MODEL_PATH = os.path.join(BASE_DIR, "../trained_models_stress_physics_mamba/best_stress_full_dual.pth")
DEFAULT_OUTPUT_DIR = os.path.join(BASE_DIR, "../evaluation_results_stress_mamba")
...
class SequentialStressDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # [FIX] Added 'os.path.basename' to return 6 items
...
def evaluate(args):
    print("========================================================")
    print("   Physics-Informed STRESS Model Evaluation (Final)     ")
    print("========================================================")
    ...
    if os.path.exists(stats_path):
        ...
        stats = torch.load(stats_path, map_location='cpu') # Fixed: CPU Load
        mean = stats['mean']
        std = stats['std']
    else:
    ...
    except: print("Failed to load model weights."); return
    model.eval()
    ...
    if args.all:
        ...
        with torch.no_grad():
            for inputs, targets, targets_prev, dists, phys_params, _ in tqdm(val_loader):
                ...
                pred_clean[pred_clean < 0.05] = 0.0 # Denoise
        ...
        for inputs, targets, _, _, _, fname in val_loader:
            ...
            plt.savefig(os.path.join(args.output_dir, f"stress_vis_sample_{count}.png"))
            plt.close()
            print(f"Saved visualization: stress_vis_sample_{count}.png")
            count += 1
