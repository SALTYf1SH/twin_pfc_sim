...
def main():
    ...
    def load_mamba_variant(name, filename, branch_mode, fallback_dir=None):
        ...
        if name == "Proposed" and not os.path.exists(path):
             path = os.path.join(MODEL_DIR, "best_subsidence_full_dual_seed53.pth") 
             if not os.path.exists(path):
                 path = os.path.join(MODEL_DIR, "best_subsidence_full_dual.pth")
    ...
    load_mamba_variant("Proposed", "best_subsidence_full_dual_seed53.pth", 'dual')

    # 2. No Physics
    load_mamba_variant("No Physics", "best_subsidence_no_physics_dual_no_phys_seed53.pth", 'dual')
    
    # 3. Static Only
    load_mamba_variant("Static Only", "best_subsidence_full_static_only_seed53.pth", 'static_only')
    
    # 4. Dynamic Only
    load_mamba_variant("Dynamic Only", "best_subsidence_full_dynamic_only_seed53.pth", 'dynamic_only')
    ...
    # Use the first model (Proposed) for selection logic
    selection_model = models[0]['model']
    ...
    n_cols = 1 + len(models) # GT + Ablations
    ...
    for r, sample in enumerate(top_samples):
        ...
        if r == n_rows - 1:
             ax_gt.set_xlabel("(a) Ground Truth", fontweight='bold')
        ...
        for c, m_cfg in enumerate(models):
            ...
            if r == n_rows - 1:
                ax.set_xlabel(f"({chr(98+c)}) {m_cfg['name']}", fontweight='bold', fontsize=9)
    ...
    save_path = os.path.join(OUTPUT_DIR, "ablation_results_sci.svg")
