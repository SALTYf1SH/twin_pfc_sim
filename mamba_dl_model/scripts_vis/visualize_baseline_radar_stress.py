...
# Data Path for Stress
CSV_PATH_MAMBA = os.path.join(BASE_DIR, "../evaluation_results_stress_mamba", "baseline_comparison_summary.csv")
...
def plot_radar(df, metrics, output_path):
    ...
    for idx, row in df.iterrows():
        ...
        if "Baseline (CNN)" in label: label = "CNN"
        if "Baseline (LSTM)" in label: label = "LSTM"
...
def main():
    ...
    if data_path is None:
        print("Warning: No Stress Baseline CSV found. Generating Placeholder Data.")
        data = {
            "Model": ["Proposed (Mamba)", "Transformer (Dual)", "Baseline (CNN)", "Baseline (LSTM)"],
            "MSE": [0.0003, 0.00045, 0.0006, 0.00035], 
            "Evo": [0.39, 0.43, 0.50, 0.38],
            "PCR": [0.63, 0.60, 0.59, 0.58],
            # Adding hypothetical metrics if simpler summary doesn't have them, 
            # script will filter automatically.
            "SSIM": [0.95, 0.93, 0.88, 0.90],
            "PCC": [0.98, 0.97, 0.94, 0.95]
        }
    ...
    # 2. Config Metrics (Available in Stress Eval)
    metric_config = {
        "MSE": -1,
        "MAE": -1,
        "SSIM": 1,
    ...
    # 4. Plot
    output_path = os.path.join(OUTPUT_DIR, "stress_radar_comparison.svg")
