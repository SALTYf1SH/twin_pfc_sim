...
# Data Path for Subsidence Ablation
CSV_PATH_MAMBA = os.path.join(BASE_DIR, "../evaluation_results_subsidence_mamba", "ablation_summary_mamba.csv")
...
def main():
    ...
    if data_path is None:
        print("Warning: No Subsidence Ablation CSV found. Generating Placeholder Data.")
        data = {
            "Model": ["Proposed (Mamba)", "No Physics", "Static Only", "Dynamic Only"],
            "MSE": [0.00088, 0.00112, 0.01152, 0.00264], 
            "Evo": [0.73, 0.83, 2.67, 1.27],
            "PCR": [0.72, 0.71, 0.47, 0.68],
            # Hypothetical
            "SSIM": [0.93, 0.92, 0.75, 0.85],
            "PCC": [0.97, 0.96, 0.80, 0.90]
        }
    ...
    # 4. Plot
    output_path = os.path.join(OUTPUT_DIR, "subsidence_ablation_radar_comparison.svg")
