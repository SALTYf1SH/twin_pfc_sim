...
def main():
    ...
    with torch.no_grad():
        for x, y, s_id, st_id, dist, phys_params in loader:
            ...
            gt_bin = (y.view(1, 1, 64, 64) > 0.1).float() 
    ...
    # 3 Columns: GT, Pred, Error
    n_rows = len(top_samples); n_cols = 3 
    ...
    vmin, vmax = 0.0, 1.2
    # Error Vmax? Maybe 0.3 or 0.5 depending on error calc
    # Generally error should be small, so 0.3 is reasonable to show distribution
    vmin_e, vmax_e = 0.0, 0.3
    ...
    for r, sample in enumerate(top_samples):
        ...
        # Pred
        with torch.no_grad():
             pred = model_mamba(x_in).view(64, 64).cpu().numpy()
             
        # Error (Absolute Difference)
        error_map = np.abs(pred - y_gt)
        
        # 1. Ground Truth
        ax_gt = axes[r, 0]
        ax_gt.imshow(y_gt, cmap='jet', vmin=vmin, vmax=vmax, origin='lower')
        ...
        # 2. Prediction
        ax_pred = axes[r, 1]
        ax_pred.imshow(pred, cmap='jet', vmin=vmin, vmax=vmax, origin='lower')
        ax_pred.set_xticks([]); ax_pred.set_yticks([])
        if r == n_rows - 1: ax_pred.set_xlabel("(b) Prediction (Proposed)", fontweight='bold')
        
        # 3. Error
        ax_err = axes[r, 2]
        im_err = ax_err.imshow(error_map, cmap='jet', vmin=vmin_e, vmax=vmax_e, origin='lower')
        ax_err.set_xticks([]); ax_err.set_yticks([])
        if r == n_rows - 1: ax_err.set_xlabel("(c) Prediction Error", fontweight='bold')
            
    plt.tight_layout(pad=0.5, w_pad=0.2, h_pad=0.2)
    save_path = os.path.join(OUTPUT_DIR, "stress_error_distribution.svg")
