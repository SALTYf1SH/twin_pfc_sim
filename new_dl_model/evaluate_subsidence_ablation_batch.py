...
SUMMARY_CSV = os.path.join(OUTPUT_DIR, "ablation_summary_filtered.csv")
...
def evaluate_loop(exp_config, val_loader, device, feature_dims, min_gt_pcr):
    ...
    model = DualBranchModel(feature_dims['static'], feature_dims['dynamic'], OUTPUT_FEATURES, branch).to(device)
