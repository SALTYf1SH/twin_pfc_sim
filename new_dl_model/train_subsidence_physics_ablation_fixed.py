...
FIXED VERSION: KS-Dynamics, Correct Dataset Shapes, Linear Output.

Usage Examples:
1. Full Model: python train_subsidence_physics_ablation.py --ablation full --branch_mode dual
2. No Arch:    python train_subsidence_physics_ablation.py --ablation no_arch
"""
...
OUTPUT_DIR = os.path.join(BASE_DIR, "trained_models_subsidence_ablation") 
...
class PhysicsInformedLoss(nn.Module):
    def __init__(self, weights):
        ...
        self.ssim = SSIMLoss()
        self.tv = TVLoss()
        # [Fix] Use new Theory Consistent Loss
...
class SequentialFractureDataset(Dataset):
    ...
    def _parse_filename(self, filepath):
        ...
        s_pos = filename.rfind("sample"); s_start = s_pos + len("sample") + 1; s_id = int(filename[s_start : s_start + 4])
        st_pos = filename.rfind("step"); st_start = st_pos + len("step") + 1; st_id = int(filename[st_start : st_start + 3])
        return s_id, st_id
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=500):
        ...
        # [Fix] Define local variable first
...
def main():
    ...
    if not os.path.exists(dataset_path): print("No files found"); return
    ...
    temp_dataset = SequentialFractureDataset(all_files, PARAMS_JSON_PATH, global_index_map=global_index_map)
    ...
    stats_path = os.path.join(OUTPUT_DIR, "subsidence_stats_ablation.pt")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    ...
    train_ds = SequentialFractureDataset(train_files, PARAMS_JSON_PATH, transform=transform, global_index_map=global_index_map)
    val_ds = SequentialFractureDataset(val_files, PARAMS_JSON_PATH, transform=transform, global_index_map=global_index_map)
