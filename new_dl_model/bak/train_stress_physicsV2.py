# -*- coding: utf-8 -*-
"""
Main script for training the PHYSICS-INFORMED Stress-Based Surrogate Model.
(Integrated with KS-Dynamics Physics)

Key Fixes:
1. Fixed Dataset Reshape/Transpose logic (x is 1D, y is 2D Transposed).
2. Integrated TheoryConsistentLoss with ODE solver for Key Strata.
3. Fixed PositionalEncoding local variable error.
4. Added missing IMG_SIZE definition.
"""
...
OUTPUT_DIR = os.path.join(BASE_DIR, "trained_models_stress_physics")
# 关键：加载应力模型的物理参数文件 (应包含 ks_heights 等新参数)
PARAMS_JSON_PATH = os.path.join(BASE_DIR, "stress_para/stress_physics_params.json")

# Model Hyperparameters
STATIC_FEATURES = 17      # 应力数据集包含 17 个静态岩石力学参数
OUTPUT_HEIGHT = 64
OUTPUT_WIDTH = 64
IMG_SIZE = 64             # [Fix] Added missing definition
...
LAMBDA_SSIM = 0.3         
LAMBDA_TV = 1e-5          
LAMBDA_ARCH = 0.5         # 动态活动拱权重
LAMBDA_EVO = 0.2          # 演化一致性权重
...
# --- [Fix] New Theory Consistent Loss with ODE ---
...
class PhysicsInformedLoss(nn.Module):
    ...
    def forward(self, pred_flat, target_flat, target_prev_flat, mining_dists, phys_params):
        # [Fix] Flatten target for MSE consistency
        target_flat_vec = target_flat.view(target_flat.size(0), -1)
        
        # Reshape for Image-based Losses
...
class SequentialStressDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with np.load(curr_path) as data:
            ...
            # [Fix] Reshape Logic
            # x_t is 1D Feature Vector -> No Reshape
            # y_t is Target Image -> Reshape if flat + Transpose
            if y_t.ndim == 1: 
                y_t = y_t.reshape(IMG_SIZE, IMG_SIZE)
            y_t = y_t.T # Transpose to align (Height, Length)
        ...
        # Load Physics Params (9 params for KS model)
        default_params = {
...
def train_model(model, train_loader, val_loader, criterion, optimizer, device):
    print("\nStarting Physics-Informed Stress Model Training...")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if (val_loss/n_val) < best_val_loss:
            ...
            path = os.path.join(OUTPUT_DIR, "best_stress_physics_model.pth")
...
def main():
    ...
    if not os.path.exists(PARAMS_JSON_PATH):
        ...
        print("Please run 'fit_stress_params.py' first!")
        return
