...
支持通过命令行参数进行消融实验，并自动将实验结果追加记录到 CSV 文件中。

Usage Examples:
1. 训练完整模型: python train_stress_physics_ablation.py --ablation full --branch_mode dual
2. 训练无活动拱: python train_stress_physics_ablation.py --ablation no_arch
...
"""
...
OUTPUT_DIR = os.path.join(BASE_DIR, "trained_models_stress_ablation") # 模型保存目录
PARAMS_JSON_PATH = os.path.join(BASE_DIR, "stress_para/stress_physics_params.json")
LOG_FILE = os.path.join(OUTPUT_DIR, "experiment_log.csv") # <--- 结果记录文件
...
class DualBranchModel(nn.Module):
    def __init__(self, static_size, dynamic_size, output_size, branch_mode='dual',
        ...
        self.branch_mode = branch_mode # 'dual', 'static_only', 'dynamic_only'
        ...
        # ============================================================
        # [修正] 明确定义 fusion_input_size
        # 即使在消融模式下，我们也是通过输入 0 张量来屏蔽支路，
        # 因此融合层的输入维度始终保持为 (Static输出 + Dynamic输出)
        # ============================================================
...
def train_model(model, train_loader, val_loader, criterion, optimizer, device, save_name):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # print(f"Epoch {epoch+1}: Train Loss={acc_loss/len(train_loader):.4f} | Val Loss={val_loss/n_val:.4f}")
...
def main():
    ...
    # 5. 记录结果到 CSV
    # 字段: Timestamp, Ablation_Mode, Branch_Mode, Best_Val_Loss, Model_Path
