# -*- coding: utf-8 -*-
"""
Batch Evaluation Script for SUBSIDENCE Model Ablation Study
(Includes True Evolution Error & PCR Metrics)

功能：
1. 遍历所有沉降消融实验模型 (best_subsidence_*.pth)。
2. 在验证集上计算 MSE, SSIM, PCR (物理合规率) 和 True_Evo_Error (真实演化误差)。
3. 汇总结果至 CSV。

输出：
evaluation_results_subsidence/ablation_summary.csv
"""
...
DATASET_DIR = os.path.join(BASE_DIR, "../final_dataset") # 沉降数据集
MODEL_DIR = os.path.join(BASE_DIR, "trained_models_subsidence_ablation")
OUTPUT_DIR = os.path.join(BASE_DIR, "evaluation_results_subsidence")
...
STATIC_FEATURES = 11      # [关键差异] 沉降模型为 11 个静态特征
OUTPUT_HEIGHT = 64
OUTPUT_WIDTH = 64
...
# --- 2. 辅助类：ActivityArchMaskGenerator ---
    ...
    def __getitem__(self, idx):
        ...
        if st_id > 1:
            ...
            if prev_idx is not None:
                ...
                y_prev = y_t.clone(); x_prev = x_t.clone()
        else:
            y_prev = torch.zeros_like(y_t); x_prev = torch.zeros_like(x_t)
        ...
        if self.transform:
            ...
            if st_id > 1: x_prev = self.transform(x_prev)
...
def evaluate_single_experiment(exp_config, val_loader, device, feature_dims):
    ...
    model_name = f"best_subsidence_{ablation}_{branch_mode}.pth" # [注意] 文件名前缀是 subsidence
    ...
    results = {
        ...
        "True_Evo_Error": np.mean(all_true_evo),
        "PCR": np.mean(all_pcr)
    }
...
def main():
    print("========================================================")
    print("   Batch Ablation Evaluation (Subsidence) - FIXED       ")
    print("========================================================")
    ...
    temp_loader = DataLoader(SequentialFractureDataset(train_files[:200], ""), batch_size=100)
    all_x = [x for x, _, _, _, _, _ in temp_loader] 
