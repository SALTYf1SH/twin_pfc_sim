# -*- coding: utf-8 -*-
"""
Baseline Training Script for STRESS-Based Inversion (EAAI Comparison).
Models: 
  1. Deep CNN (ResNet-Generator Style) - Representative of Convolutional Networks
  2. Bi-LSTM (Sequence-to-Image) - Representative of Recurrent Networks

Features:
- STATIC_FEATURES = 17 (Specific to Stress Dataset)
- Auto-detects input dimension to prevent shape mismatch errors
- Pure Data-Driven (MSE Loss only)
"""
...
# Params json is not used for training logic here, but kept for compatibility
...
# SSIM Metric for Evaluation Logging
...
class DeepCNNBaseline(nn.Module):
    """
    Baseline A: Deep CNN (Generator Style)
    """
    def __init__(self, input_size, output_size=4096):
        super(DeepCNNBaseline, self).__init__()
        
        # Project 1D input to 2D feature map (128 x 8 x 8)
        ...
        self.conv_blocks = nn.Sequential(
            ...
            # No sigmoid, linear output for regression
        )
...
class BiLSTMBaseline(nn.Module):
    """
    Baseline B: Bi-Directional LSTM
    """
    def __init__(self, dynamic_len, static_len, output_size=4096):
        ...
        # Input dim = 1 (pressure value) + 17 (static features repeated)
    ...
    def forward(self, x):
        # x: [Batch, Total_Dim]
        # Split features based on known static length
        x_static = x[:, :self.static_len]
        x_dynamic = x[:, self.static_len:]
        
        # Prepare Sequence
        # Dynamic: [Batch, Dyn_Len, 1]
        seq_dynamic = x_dynamic.unsqueeze(-1) 
        # Static: [Batch, 1, Stat_Len] -> Repeat -> [Batch, Dyn_Len, Stat_Len]
        seq_static = x_static.unsqueeze(1).repeat(1, self.dynamic_len, 1) 
        
        # Concat: [Batch, Dyn_Len, 1 + Stat_Len]
...
def train_baseline(model, train_loader, val_loader):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if avg_val_ssim > best_ssim:
            ...
            save_path = os.path.join(OUTPUT_DIR, f"best_baseline_stress_{MODEL_TYPE}.pth")
...
def main():
    ...
    train_ds = SequentialStressDataset(train_files, None, transform=transform)
    val_ds = SequentialStressDataset(val_files, None, transform=transform)
    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True, num_workers=0)
    val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=0)
    
    # 2. Initialize Model with REAL DIMENSIONS
    if MODEL_TYPE == "CNN":
