# -*- coding: utf-8 -*-
"""
Physics-Informed STRESS Model Evaluation (With GT-PCR Filtering)

New Feature:
--min_gt_pcr [float]: Threshold (0.0 ~ 1.0).
Samples where Ground Truth violates the physics mask (GT-PCR < threshold) 
will be AUTOMATICALLY SKIPPED during evaluation.
"""
...
DEFAULT_OUTPUT_DIR = os.path.join(BASE_DIR, "evaluation_results_stress_filtered")
...
# --- 4. Evaluation Main (With Filtering) ---

def evaluate(args):
    print("========================================================")
    print("   Physics-Informed STRESS Model Evaluation (Filtered)  ")
    print(f"   Filtering Threshold: GT-PCR >= {args.min_gt_pcr}")
    print("========================================================")
    ...
    if os.path.exists(stats_path):
        ...
        stats = torch.load(stats_path, map_location='cpu') # CPU Load
        mean = stats['mean']
        std = stats['std']
    else:
        print("[WARNING] Stats file not found! Fallback calculation.")
        temp_dataset = SequentialStressDataset(all_files[:200], "") 
    ...
    except: print("Failed to load model."); return
    model.eval()
    
    # 4. Filtered Evaluation
    ...
    if args.all:
        ...
        with torch.no_grad():
            for inputs, targets, targets_prev, dists, phys_params, _ in tqdm(val_loader):
                ...
                # Binarize GT for stress (Threshold 0.1)
                ...
                # --- Step 2: Filtering ---
                # Keep samples where GT-PCR >= threshold
                ...
                if not keep_indices.any():
                    continue # Skip whole batch if no good samples
                ...
                # --- Step 3: Model Inference on Filtered Data ---
        ...
        print("\n=== Stress Model Performance (Filtered) ===")
        print(f"Total Validation Samples: {total_samples}")
        print(f"Samples Kept (GT-PCR >= {args.min_gt_pcr}): {kept_samples} ({kept_samples/total_samples*100:.1f}%)")
        ...
        if kept_samples > 0:
            ...
            print(f"PCR : {np.mean(all_metrics['PCR']):.4f}")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--all", action="store_true", help="Run full stats")
