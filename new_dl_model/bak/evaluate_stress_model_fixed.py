# -*- coding: utf-8 -*-
"""
Physics-Informed STRESS Model Evaluation Script (FINAL FIXED)

Crucial Fixes:
1. [Normalization] Added Zero-Variance Protection to prevent input explosion (MSE e+24).
2. [Dataset] Corrected shape handling (1D input, Transposed 2D target).
3. [Physics] Integrated ODE-based Mask Generator for accurate PCR.
"""
...
def evaluate(args):
    print("========================================================")
    print("   Physics-Informed STRESS Model Evaluation (Enhanced)  ")
    print("========================================================")
    ...
    # 2. Normalization Stats (With Zero-Variance Protection)
    print("Calculating stats...")
    # Use empty string for JSON path as we only need x
    temp_dataset = SequentialStressDataset(all_files[:200], "") 
    temp_loader = DataLoader(temp_dataset, batch_size=32)
    
    all_x = []
    # [FIX] Unpack correctly (dataset returns 6 items)
    ...
    # [FIX] Protect against division by zero (Exploding Gradients/MSE)
    std[std < 1e-6] = 1.0
    print(f"Stats check - Max Std: {std.max():.2f}, Min Std: {std.min():.6f}")
