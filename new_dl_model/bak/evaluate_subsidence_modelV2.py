# -*- coding: utf-8 -*-
"""
Physics-Informed Subsidence Model Evaluation Script

该脚本专为 '沉降-裂隙场' 物理感知反演模型设计。
配置已适配沉降数据集 (11个静态特征) 和无 Sigmoid 的线性输出模型。

主要功能：
1. 加载训练好的沉降反演模型 (best_subsidence_physics_model.pth)。
2. 评估物理一致性指标：演化误差 (Evo Error)。
3. 执行统计假设检验。
4. 生成可视化对比图 (自动截断 [0,1] 范围以适应线性输出)。

用法示例:
# 1. 少量样本可视化
python new_dl_model/evaluate_subsidence_model.py --num_samples 5

# 2. 全量统计评估
python new_dl_model/evaluate_subsidence_model.py --all
"""
...
# 5. 模型特征参数
STATIC_FEATURES = 11      # 沉降模型为 11 个静态参数
OUTPUT_HEIGHT = 64
OUTPUT_WIDTH = 64
...
class SequentialFractureDataset(Dataset):
    """
    适配沉降模型的数据集。
    需要加载物理参数以保持接口一致 (虽然评估时可能不直接使用物理参数计算Loss)。
    """
    def __init__(self, npz_file_list, params_json_path, transform=None):
        ...
        if os.path.exists(params_json_path):
            ...
            # 评估时如果找不到参数文件，给默认值，不影响 MSE/SSIM 计算
            self.physics_params = {} 
...
def evaluate(args):
    print("========================================================")
    print("   Physics-Informed SUBSIDENCE Model Evaluation         ")
    print("========================================================")
    ...
    # 使用空的 json path 因为只用它来加载 x 计算均值
    ...
    dynamic_feats = total_feats - STATIC_FEATURES # 11
    ...
    # 3. 执行评估
    val_dataset = SequentialFractureDataset(val_files, PARAMS_JSON_PATH, transform=transform)
    ...
    if args.all:
        ...
        print("\n======================================================")
        print("             Statistical Summary (Subsidence)         ")
        print("======================================================")
        ...
        # 使用蓝色调区分沉降模型
        ax.hist(mse_arr, bins=100, color='#1f77b4', alpha=0.8, edgecolor='black', linewidth=0.5)
        ax.set_title('MSE Distribution (Subsidence Model Validation)', fontsize=14)
        ax.set_xlabel('MSE', fontsize=12)
        ax.set_ylabel('Frequency', fontsize=12)
        ax.grid(True, linestyle='--', alpha=0.3)
        
        hist_path = os.path.join(args.output_dir, "subsidence_mse_histogram.png")
        ...
        with torch.no_grad():
            for inputs, targets, targets_prev, fnames in val_loader:
                ...
                plt.suptitle(f"Subsidence-Based | File: {fnames[0]}\nMSE: {mse:.5f}", fontsize=14)
                ...
                save_path = os.path.join(args.output_dir, f"subsidence_sample_{idx}.png")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Physics-Informed Subsidence Model")
