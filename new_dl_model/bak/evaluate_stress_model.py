...
配置已适配红庆河工况参数 (17个静态特征) 和应力数据集。

主要功能：
1. 加载训练好的应力反演模型 (best_stress_physics_model.pth)。
2. 评估物理一致性指标：演化误差 (Evo Error)。
3. 执行统计假设检验 (对数正态分布, 异常值分析)。
4. 生成可视化对比图 (Ground Truth vs Prediction vs Difference)。
...
# 4. 模型特征参数 (关键差异点)
STATIC_FEATURES = 17      # 红庆河工况静态参数数量
OUTPUT_HEIGHT = 64
OUTPUT_WIDTH = 64
...
def evaluate(args):
    ...
    if not os.path.exists(DATASET_DIR):
        ...
        print("Please assure you pointed to 'final_dataset_stress'")
        return
    ...
    dynamic_feats = total_feats - STATIC_FEATURES # 这里 STATIC_FEATURES 是 17
    ...
    # 3. 执行评估
    val_dataset = SequentialStressDataset(val_files, transform=transform)
    ...
    if args.all:
        ...
        ax.hist(mse_arr, bins=100, color='#e377c2', alpha=0.8, edgecolor='black', linewidth=0.5) # 使用不同颜色(粉色)区分应力模型
