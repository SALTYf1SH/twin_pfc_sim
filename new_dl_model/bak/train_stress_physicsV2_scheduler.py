...
class EvolutionLoss(nn.Module):
    ...
    def forward(self, pred_t, target_t, target_prev):
        ...
        # [Fix] Bounded Relative Error (Tanh)
        # Prevents explosion when GT is small, limits range to [0, 1)
        diff_sq = torch.sum((pred_delta - true_delta) ** 2)
        norm_sq = torch.sum(true_delta ** 2)
        
        rel_error = diff_sq / (norm_sq + 1e-8)
        return torch.tanh(rel_error)
...
def main():
    ...
    # If using LambdaLR, we define a function that returns a multiplicative factor
    # Phase 1: Warmup (0 to WARMUP_EPOCHS) -> factor increases from 0 to 1 (or specific start to max)
    # Phase 2: Cosine (WARMUP_EPOCHS to NUM_EPOCHS) -> factor decays from 1 to 0 (or min)
    
    # Actually, let's use a simpler consistent definition.
    # We want LR to go from 0 -> MAX_LR -> MIN_LR
    # So we set optimizer initialized with MAX_LR
    ...
    def lr_lambda(epoch):
        if epoch < WARMUP_EPOCHS:
            ...
            # Cosine decay: 1 -> 0 (approx, to MIN_LR ratio)
