...
(FINAL FIXED VERSION: Full Stats, KS-Dynamics, Correct Shapes)

Key Features:
1. Calculates Mean/Std on the ENTIRE STRESS dataset to prevent MSE explosion.
2. Saves 'stress_stats.pt' for consistent evaluation.
3. Uses TheoryConsistentLoss (ODE-based) for Key Strata dynamics.
4. Corrects Dataset shapes (x: 1D, y: 2D Transposed).
"""
...
# [Config] Stress Model Hyperparameters
...
def train_model(model, train_loader, val_loader, criterion, optimizer, device):
    print("\nStarting Physics-Informed STRESS Training...")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(f"Epoch {epoch+1}: Train Loss={acc_loss/n_train:.4f} "
              f"(MSE={acc_mse/n_train:.4f}, SSIM={acc_ssim/n_train:.3f}) | Val Loss={val_loss/n_val:.4f}")
...
def main():
    ...
    # [CRITICAL FIX] Normalization on FULL STRESS DATASET
    ...
    # Using larger batch size for faster scanning
