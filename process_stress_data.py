# -*- coding: utf-8 -*-
"""
Process New Data (Stress-Based) and Merge into Final Dataset

This script processes raw PFC simulation results and merges them into a 
new dataset in the 'final_dataset_stress' directory.

It is designed to work with the 'HQH' simulation runs (23-layer model, 
17 sampled parameters).

- Input (X): A 1D vector containing 17 key rock properties and the floor stress vector.
- Output (Y): A 1D vector representing a flattened 2D grid of fracture intensity.

How to run:
python process_stress_data.py Z:\pfc_2d_sim_hqh
"""
...
# Directory where the final PROCESSED STRESS-BASED dataset will be saved.
FINAL_DATASET_DIR = "final_dataset_stress"
...
def get_simulation_parameters_hqh(sim_folder_path):
    """
    Reads the key varying parameters from the simulation's parameter file.
    This version is updated for the 17 parameters varied in 
    'parameter_sampler_hqh.py'.
    """
    ...
    # Extract all 17 varied parameters from the HQH config
    param_vector = []
    
    try:
        # Sandstone (TYPE 0) properties (4)
        ...
        # Gritstone (TYPE 1) properties (4)
        param_vector.append(config["ROCK_PARA"][1][3][1]) # gritstone_emod
        param_vector.append(config["ROCK_PARA"][1][4][1]) # gritstone_pb_ten
        param_vector.append(config["ROCK_PARA"][1][5][1]) # gritstone_pb_coh
        param_vector.append(config["ROCK_PARA"][1][7][1]) # gritstone_kratio

        # Mudstone/Siltstone (TYPE 2) properties (3)
        ...
        # Coal (TYPE 3) properties (3)
        param_vector.append(config["ROCK_PARA"][3][3][1]) # coal_emod
        param_vector.append(config["ROCK_PARA"][3][4][1]) # coal_pb_ten
        param_vector.append(config["ROCK_PARA"][3][5][1]) # coal_pb_coh

        # Key Stratum Thicknesses (3)
        param_vector.append(config["ROCK_LAYER_THICKNESSES"][5])  # main_ks_thickness (L17 - middle part)
        param_vector.append(config["ROCK_LAYER_THICKNESSES"][12]) # coal_seam_thickness (L11)
        param_vector.append(config["ROCK_LAYER_THICKNESSES"][13]) # immediate_floor_thickness (L10)
    
    except IndexError as e:
        print(f"ERROR: Failed to parse parameters for {sim_folder_path}.")
        print("This is likely due to a mismatch between the script's hard-coded paths")
        print("and the 'simulation_parameters.txt' file structure.")
        raise e
...
def process_simulation_folder(sim_folder_path, output_dir):
    ...
    This version uses FLOOR STRESS instead of subsidence.
    """
    try:
        # 1. Get the fixed rock property vector (17 params) for this simulation
        rock_prop_vector = get_simulation_parameters_hqh(sim_folder_path)
        ...
        # 2. Load the FLOOR STRESS data for all steps
        # --- THIS IS THE KEY MODIFICATION ---
        stress_csv_path = os.path.join(sim_folder_path, "floor_yy_stress_vs_section.csv")
        if not os.path.exists(stress_csv_path):
            print(f"  - WARNING: Floor Stress CSV '{os.path.basename(stress_csv_path)}' not found in {os.path.basename(sim_folder_path)}. Skipping folder.")
            return 0
        stress_df = pd.read_csv(stress_csv_path)
        # --- END MODIFICATION ---
        
        # The first column is the monitoring point position, the rest are steps
        step_columns = stress_df.columns[1:]
        ...
        for i, step_col_name in enumerate(step_columns):
            ...
            # a. Get the STRESS vector for the current step
            # --- THIS IS THE KEY MODIFICATION ---
            stress_vector = stress_df[step_col_name].values.astype(np.float32)
            
            # b. Combine to form the full input vector X
            input_vector_X = np.concatenate([rock_prop_vector, stress_vector])
            # --- END MODIFICATION ---
            ...
            if os.path.exists(output_filepath):
                # This check is kept from the original script
                continue
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Process new PFC simulation data (Rock Params + Stress) and merge into the final dataset.")
    parser.add_argument("source_dir", type=str, help="The absolute path to the new directory containing raw simulation folders (e.g., the ..._hqh results).")
    args = parser.parse_args()

    source_experiments_dir = args.source_dir

    print("======================================================")
    print("      Process and Merge PFC Data (Rock + Stress)      ")
    print("======================================================")
    ...
    for folder_path in tqdm(sim_folders, desc="Processing Stress-Based Sims"):
    ...
    print(f"Dataset created in: '{FINAL_DATASET_DIR}'")
    print("======================================================")
