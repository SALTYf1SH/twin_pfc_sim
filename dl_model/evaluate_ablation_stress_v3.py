...
ablation models (Full V3, Static-Only, Dynamic-Only) on the entire test set.

This script is updated to load the new 'DualBranchModel_v3' 
(Late Fusion + ConvDecoder) and compare it against the two single-branch 
ablation models.
"""
...
# Paths are relative to the project root (where you run the script from)
SCRIPT_DIR = os.path.dirname(__file__)
TRAINED_MODELS_DIR = "trained_models_stress" 

# --- _MODIFIED_ (Path to the new V3 model) ---
DUAL_BRANCH_V3_MODEL_PATH = os.path.join(TRAINED_MODELS_DIR, "best_stress_model_v3.pth")
...
# --- [MODIFIED] Model 1: The new V3 Full Model ---
class DualBranchModel_v3(nn.Module):
    """ (Copied from train_stress_model_v3.py) """
    ...
    def forward(self, x):
        ...
        latent = self.decoder_input(fused)
...
def main():
    print("======================================================")
    print("     Ablation Models Evaluation Script (Stress V3)    ") # <-- _MODIFIED_
    print("======================================================")
    ...
    try:
        ...
        print("Please run all three training scripts (v3, static-only, dynamic-only) first.")
        return
    ...
    try:
        # 1. Full Model (V3)
        # --- _MODIFIED_ (Load V3 model) ---
        full_model = DualBranchModel_v3(static_size=STATIC_FEATURES, dynamic_size=dynamic_size, output_size=OUTPUT_FEATURES).to(device)
        full_model.load_state_dict(torch.load(DUAL_BRANCH_V3_MODEL_PATH, map_location=device))
        full_model.eval()
        print(f"Full model (V3) loaded from {DUAL_BRANCH_V3_MODEL_PATH}")
        # --- _END_MODIFIED_ ---
        ...
        print("Please ensure you have trained all three models (v3, static-only, dynamic-only) first.")
        return
    ...
    print("\n======================================================")
    print("        Ablation Study Results (Stress Model V3)      ") # <-- _MODIFIED_
    print("======================================================")
    
    print("--- Full Model (V3 - Late Fusion + ConvDecoder) ---") # <-- _MODIFIED_
