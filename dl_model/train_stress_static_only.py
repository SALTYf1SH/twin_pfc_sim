# -*- coding: utf-8 -*-
"""
Script for training a STATIC-ONLY ablation model (Stress-Based).

This model uses only the static MLP branch from the dual-branch model
to predict the fracture field based solely on the 17 geological parameters
...
OUTPUT_DIR = "trained_models_stress"      # <-- _MODIFIED_

STATIC_FEATURES = 17 # <-- _MODIFIED_ (Was 11)
...
class StaticDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with np.load(filepath) as data:
            ...
            # This logic works perfectly, as STATIC_FEATURES is now 17
...
class StaticOnlyModel(nn.Module):
    ...
    (This is identical to the one in train_stress_model.py)
    """
    def __init__(self, static_size, output_size, dropout=0.1):
        ...
        self.prediction_head = nn.Sequential(
            nn.Linear(32, 1024), # Input matches static_branch output (32)
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(1024, 2048),
            nn.ReLU(),
...
def train_model(model, train_loader, val_loader, criterion, optimizer, device):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            model_path = os.path.join(OUTPUT_DIR, "best_stress_model_static_only.pth")
...
def main():
    print("========================================================")
    print("      Static-Only (Stress) Ablation Model Training      ") # <-- _MODIFIED_
    print("========================================================")
    ...
    # This dataset correctly loads only the static part
    stats_dataset = StaticDataset(train_files, transform=None) 
    ...
    # We save these stats separately
    np.savez_compressed(
        os.path.join(os.path.dirname(__file__), "normalization_stats_static_stress.npz"), 
        mean=mean.numpy(), 
        std=std.numpy()
    )
    print("Normalization stats calculated and saved.")
